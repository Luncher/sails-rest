...
registerConnection: function(connection, collections, cb) {
  ...
  config.actionMapping = sails.config.restadapter || {}      
...
findOne: function (connection, collection, options, cb) {
  return Connection.findOne(connections[connection], collection, options, cb);
},
 ...
 * @param {Function} cb - function to call with query results.
 */
patch: function (connection, collection, options, values, cb) {
  if (!cb) {
    const promise = deferred()
    Connection.patch(connections[connection], collection, options, values, promise.defer())
    return promise
  } else {
    return Connection.patch(connections[connection], collection, options, values, cb)
  }
},

action: function (connection, collection, options, values, cb) {
  if (!cb) {
    const promise = deferred()
    Connection.action(connections[connection], collection, options, values, promise.defer());
    return promise
  } else {
    return Connection.action(connections[connection], collection, options, values, cb);
  }   
}
};
