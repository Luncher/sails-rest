...
if (action && identity) {
    ...
    if (!keys || keys.length == 0) {
      config.endpoint = url.resolve(conn.connection.endpoint + '/' + basepath, path)
    } else {
      ...
      config.endpoint = url.resolve(conn.connection.endpoint + '/' + basepath, prefix)
    }
    ...
    config.endpoint = url.resolve(conn.connection.endpoint + '/' + basepath, it.path);
  } else {
    ...
    if(_.isObject(conn.options) && conn.options.hasOwnProperty('id')){
      config.endpoint = url.resolve(conn.connection.endpoint + '/' + basepath, prefix + '/' + conn.options.id);
