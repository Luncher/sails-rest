...
if (!keys || keys.length == 0) {
  config.endpoint = url.resolve(conn.connection.endpoint + '/', path)
} else {
  ...
  config.endpoint = url.resolve(conn.connection.endpoint + '/', prefix)
}
...
config.endpoint = url.resolve(conn.connection.endpoint + '/', it.path);
} else {
...
if(_.isObject(conn.options) && conn.options.hasOwnProperty('id')){
  config.endpoint = url.resolve(conn.connection.endpoint + '/', prefix + '/' + conn.options.id);
