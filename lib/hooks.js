...
function createEndpoint(req, method, config, conn, cb){
  const action = conn.options.$action
  const identity = conn.options.$identity
  const actionMapping = conn.connection.actionMapping

if (action && identity) {
    const path = actionMapping[identity][action].path
    const basepath = actionMapping[identity].basepath || ""    
    const keys = path.match(/\:([A-Z]+)/gi)
    if (!keys || keys.length == 0) {
      config.endpoint = url.resolve(conn.connection.endpoint + '/' + (basepath ? (basepath + "/") : ""), path)
    } else {
      const params = Object.assign({}, conn.options, conn.values)
      const prefix = keys.reduce(function(acc, cur) {
        const fieldName = cur.split(':')[1]
        return path.replace(`${cur}`, params[fieldName])
      }, path)
      config.endpoint = url.resolve(conn.connection.endpoint + '/' + (basepath ? (basepath + "/") : ""), prefix)
    }
  } else if (actionMapping[conn.collection] && actionMapping[conn.collection][method]) {
    const basepath = actionMapping[identity].basepath || ""
    const it = actionMapping[conn.collection] && actionMapping[conn.collection][method]
    config.endpoint = url.resolve(conn.connection.endpoint + '/' + (basepath ? (basepath + "/") : ""), it.path);
  } else {
    const it = actionMapping[conn.collection]
    const basepath = (it && it.basepath) ? it.basepath : ""
    const prefix = (it && it.pluralize) ? pluralize(conn.collection) : conn.collection
    if(_.isObject(conn.options) && conn.options.hasOwnProperty('id')){
      config.endpoint = url.resolve(conn.connection.endpoint + '/' + (basepath ? (basepath + "/") : ""), prefix + '/' + conn.options.id);
