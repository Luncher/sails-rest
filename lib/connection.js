  ...
  async.eachSeries(conn.connection.hooks.before, function (hook, nextHook) {
    hook(req, method, config, conn, function (err) {
      ...
      if(!_.isEmpty(config.endpoint) && typeof req === 'undefined'){
        const httpMethod = (conn.connection.methods[method] || method).toLowerCase();
...
/**
* Find record by issuing an HTTP GET request.
* @param {Object} connection - connection configuration object
* @param {String} collection - collection name. appended to API pathname.
*                              For example, given the api `http://localhost:8080/api/v1`,
*                              a collection named `user` will resolve to `http://localhost:8080/api/v1/user`.
...
* @param {Function} cb - function to call with query results.
*/
Connection.findOne = function (connection, collection, options, cb) {
  var config = {},
  req,
  conn = {
    connection: connection,
  ...
  runBeforeHooks(req, 'findOne', config, conn, function (err, req) {
    if(err) {
      return cb(err);
    }
    return req.query(conn.options).end(function (err, res) {
...
/**
* Patch record(s) by issuing an HTTP PATCH request.
* @param {Object} connection - connection configuration object
* @param {String} collection - collection name. appended to API pathname.
*                              For example, given the api `http://localhost:8080/api/v1`,
*                              a collection named `user` will resolve to `http://localhost:8080/api/v1/user`.
...
* @param {Function} cb - function to call with query results.
*/
Connection.patch = function (connection, collection, options, values, cb) {
  var config = {},
  req,
  conn = {
    connection: connection,
  ...
  runBeforeHooks(req, 'patch', config, conn, function (err, req) {
    if(err) {
      return cb(err);
    }
    return req.query(conn.options).send(conn.values).end(getResponseHandler(conn.connection, cb));
...
/**
* Patch record(s) by issuing an HTTP customize request.
* @param {Object} connection - connection configuration object
* @param {String} collection - collection name. appended to API pathname.
*                              For example, given the api `http://localhost:8080/api/v1`,
*                              a collection named `user` will resolve to `http://localhost:8080/api/v1/user`.
...
* @param {Function} cb - function to call with query results.
*/
Connection.action = function (connection, collection, options, values, cb) {
  var config = {},
  req,
  conn = {
    connection: connection,
  ...
  const identity = options.$identity = collection

  if (!connection.actionMapping[identity] 
  || !connection.actionMapping[identity][action]
  || !connection.actionMapping[identity][action].method) {
    return process.nextTick(cb, new Error(`REST Adapter Invalid identity: ${identity}, action: ${action}`))
  }

  const method = connection.actionMapping[identity][action].method
  runBeforeHooks(req, method, config, conn, function (err, req) {
    if(err) {
      return cb(err);
    }
